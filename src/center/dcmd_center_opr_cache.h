...
class DcmdCenterOprCache {
 ...
 public:
  // 添加opr cmd到cache
  void AddOprCmd(uint64_t opr_cmd_id, DcmdCenterOprCmd const& cmd);
  // 获取opr cmd cache，返回值false表示不存在
  bool GetOprCmd(uint64_t opr_cmd_id, DcmdCenterOprCmd& cmd);
  // 检查超时的opr cmd
 ...
 private:
  ...
  CwxMutexLock                       lock_;
  // 基于命令id的cache
  map<uint64_t, DcmdCenterOprCmd*>   cmd_cache_;
};
}  // dcmd
#endif
