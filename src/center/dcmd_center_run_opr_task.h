...
namespace dcmd {
class DcmdCenterApp;
// UI控制台的获取agent当前running opr的task对象
class DcmdCenterRunOprTask : public CwxTaskBoardTask {
 public:
  enum{
    TASK_STATE_WAITING = CwxTaskBoardTask::TASK_STATE_USER
  };
  DcmdCenterRunOprTask(DcmdCenterApp* app, CwxTaskBoard* taskboard):CwxTaskBoardTask(taskboard)
  {
    reply_conn_id_ = 0;
    msg_taskid_ = 0;
    app_ = app;
